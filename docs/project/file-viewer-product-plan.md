...
- [ ] F1. 定义视频与音频的支持格式清单
- [ ] F2. 实现基础 `video` renderer
- [ ] F3. 实现基础 `audio` renderer
- [ ] F4. 明确不支持转码时的回退策略
- [ ] F5. 评估是否需要封面图与时长元数据
- [ ] F6. 明确大文件媒体展示的体验边界
...
3. 再完成阶段 C
4. 再完成阶段 D
5. 完成 `pdf + image + text` 之后再评估是否进入视频和音频

原因：

- 先定协议和壳层，后续类型扩展才不会反复返工
- 先用 `PDF.js` 验证架构，再增加图片，可尽早证明方案不是单点特化
- 视频与音频牵涉更重的文件大小、编码、播放兼容性，不应过早进入

---

## 11. 当前阶段的明确停点

对当前版本，我建议先以如下停点作为第一里程碑：
...
达到这个停点后，产品上就已经形成了“统一文件展示入口”的雏形，具备对外接入价值。
...
基于当前项目状态，viewer 已完成 `pdf + image + text` 三种正式 renderer。下一轮建议继续做产品收口，而不是立刻进入视频转码类问题：

- [x] 1. 完成图片 renderer
- [x] 2. 完成纯文本 renderer 一期
- [ ] 3. 收敛 PDF.js 文案与工具栏裁剪策略
- [ ] 4. 评估并补齐 Docker 交付中的 viewer 静态资源验证

接下来优先处理 3 和 4，再决定是否进入视频与音频 renderer。
